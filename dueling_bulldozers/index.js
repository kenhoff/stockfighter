...
openOrders = []
percentSpread = .15 // percentage of last trade price
quantity = 1

positionCompensation = .05

// buy increases position
// sell decreases position
// shouldn't go above 1000 or below 1000

tradebot.levels("dueling_bulldozers").start(function(err, game) {
	if (err) {
		throw err
	}
	setInterval(() => {
		...
		tradebot.venue(game.venues[0]).stock(game.tickers[0]).quote((err, quote) => {
			...
			console.log("Position:", leftPad(position, 6), "|", "Cash:", leftPad(cash, 13), "|", "Est NAV:", leftPad(cash + (position * lastPrice), 13), "|", "Buy:", leftPad(buyPrice + " (" + (buyPrice - lastPrice) + ")", 15), "|", "Last:", leftPad(lastPrice, 8), "|", "Sell:", leftPad(sellPrice + " (+" + (sellPrice - lastPrice) + ")", 15));
